...
async function createOnlineLobby(config) {
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            // Запускаем игру с нашими настройками (размер, режим)
            // Важно: game.js должен быть готов принять этот config!
            startGame(config); 
            
            // Сразу отправляем данные в базу, чтобы зафиксировать конфиг и игроков
...
async function joinOnlineLobby(lobbyId) {
    ...
    try {
        ...
        // Определяем команду для уведомления (если game.js уже обновлен)
        let teamInfo = players[assignedId].team ? ` (Команда ${players[assignedId].team})` : "";
        alert(`✅ Вы вступили в бой!\nВаш позывной: Игрок ${window.myPlayerId}${teamInfo}`);
        
        // Загружаем игру с настройками хоста (размер карты и режим там уже есть)
        // В data.map_type у нас просто 'custom', вся соль в data.game_state
...
window.startOfflineTest = function() {
    ...
    if (mode === '2v2' && gameState.players[3]) {
