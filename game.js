...
let combatNotifTimeout = null; // Таймер для скрытия уведомления об уроне
...
function showCombatNotification(dmg, remainingHp, targetName) {
    ...
    if (remainingHp <= 0) {
        ...
        notif.style.borderColor = '#ff4444'; // Красная рамка при убийстве
    } else {
        document.getElementById('notif-hp').innerText = `Осталось ХП: ${remainingHp}`;
        notif.style.borderColor = '#ffaa00'; // Оранжевая рамка при ранении
    }

    // Показываем плашку
    notif.classList.remove('hidden');

    // Сбрасываем старый таймер, если стреляем быстро
    clearTimeout(combatNotifTimeout);
    
    // Прячем плашку через 2.5 секунды
...
function attackUnit(attacker, target) {
    ...
    if (dist <= attacker.type.attackRange) {
        ...
        // ВЫЗЫВАЕМ НАШЕ УВЕДОМЛЕНИЕ О БОЕ
...
function endTurn() {
    ...
    // Скрываем боевое уведомление принудительно, если оно висело
