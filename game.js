...
function startGame(mapType) {
    ...
    // НОВОЕ: Если это онлайн и мы Игрок 2 (Синие) - кидаем камеру в правый нижний угол
    if (window.isOnlineGame && window.myPlayerId === 2) {
        camera.x = (GRID_SIZE * TILE_SIZE) - 600; // 600 - примерная ширина экрана
...
function getVisibleMap() {
    ...
    // НОВОЕ: В онлайне смотрим только за своего игрока! В локалке - за того, чей ход.
    let viewPlayer = window.isOnlineGame ? window.myPlayerId : gameState.turn;
...
function prepareBuy(typeKey) {
    // НОВОЕ: Запрет покупки не в свой ход при онлайне
    if (window.isOnlineGame && gameState.turn !== window.myPlayerId) {
...
function handleMapClick(e) {
    ...
    // НОВОЕ: Запрет кликов по карте не в свой ход при онлайне
    if (window.isOnlineGame && gameState.turn !== window.myPlayerId) {
        return; 
    }
...
function endTurn() {
    // НОВОЕ: В онлайне нельзя завершить чужой ход!
    if (window.isOnlineGame && gameState.turn !== window.myPlayerId) {
        return;
    }
    ...
    // В локальной игре показываем экран-ширму (Буфер)
...
function updateUI() {
    ...
    // НОВОЕ: В онлайне пишем понятнее чей ход
    if (window.isOnlineGame) {
...
function renderAll() {
    ...
    let visibleMap = getVisibleMap();
    let viewPlayer = window.isOnlineGame ? window.myPlayerId : gameState.turn; // Чей туман
