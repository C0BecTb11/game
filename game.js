...
function initControls() {
    ...
    document.getElementById('btn-start-turn').onclick = startTurn; // Кнопка с буферного экрана
...
function getVisibleMap() {
    ...
    // 1. Базы всегда подсвечены для своих владельцев
            ...
            for(let dy = -v; dy <= v; dy++){
                for(let dx = -v; dx <= v; dx++){
                    ...
                    if(nx >= 0 && nx < GRID_SIZE && ny >= 0 && ny < GRID_SIZE) {
                        if(Math.abs(dx) + Math.abs(dy) <= v) vis[ny][nx] = true;
                    }
                }
            }
        }
    ...
    // 3. Обзор от захваченных точек (радиус 1 клетка)
...
function handleMapClick(e) {
    ...
    if (gameState.selectedUnit) {
        // Проверяем, пытаемся ли мы атаковать видимого врага
        if (clickedUnit && clickedUnit.owner !== gameState.turn && visibleMap[y][x]) {
            ...
            // Движение (игнорируем клики по невидимым врагам, считая клетку пустой для команды движения)
...
function endTurn() {
    ...
    // Включаем экран паузы (скрываем карту)
    ...
    // Очищаем экран, чтобы никто не подглядывал
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    updateUI();
}

// Начинаем ход (когда второй игрок взял телефон)
function startTurn() {
    gameState.state = 'IDLE';
    document.getElementById('turn-buffer').classList.add('hidden');
    renderAll(); // Рисуем карту с Туманом Войны для нового игрока
}
...
function renderAll() {
    if (gameState.state === 'BUFFER') return; // Защита: не рисуем карту во время передачи хода
    ...
    // 1. Отрисовка Тумана Войны
    for (let y = 0; y < GRID_SIZE; y++) {
        for (let x = 0; x < GRID_SIZE; x++) {
            if (!visibleMap[y][x]) {
                ctx.fillStyle = 'rgba(0, 0, 0, 0.65)'; // Темная пелена
                ctx.fillRect(x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE, TILE_SIZE);
            }
        }
    }
    
    // 2. Отрисовка зон движения и атаки
    if (gameState.selectedUnit && !gameState.selectedUnit.hasMoved) {
        ...
        for (let ty = 0; ty < GRID_SIZE; ty++) {
            for (let tx = 0; tx < GRID_SIZE; tx++) {
                ...
                // Подсвечиваем красным только если враг ВИДИМЫЙ
    ...
    // 3. Отрисовка Юнитов
    gameState.units.forEach(u => {
        // СКРЫВАЕМ ВРАГОВ В ТУМАНЕ ВОЙНЫ
